...
def parse_anno():
    ...
    # imagenames = []
    # save_mat = '../release/lip_train_test.mat'
    ...
    for package_path in ['train_set', 'val_set', 'test_set']:
        ...
        csv_path = save_root + 'lip_' + package_path + '.csv'
        anno_str_list = []
        img_id = 1
        ...
        for pa,sub,fns in os.walk(img_folder):
          ...
          for fn in fns:
            ...
            anno_list = origin_img_path.split('/')[-1] + ',' + ','.join([str(x) for x in anno] ) 
            # + ',' + str(center[0]) + ',' + str(center[1]) + ',' + str(scale)
            anno_str_list.append(anno_list)
            
        anno_str_list = sorted(anno_str_list)
        with open(csv_path, 'w') as save_csv_file:
            save_csv_file.write('\n'.join(anno_str_list))
	    # img_count  = img_id - 1
	    # print img_count
