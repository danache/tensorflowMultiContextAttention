...
RESTORE_FROM = './checkpoint/hg_new'
LOG_DIR = './logs/hg'
    ...
    for i in xrange (num_gpus):
        with tf.device('/gpu:%d' % i):
            with tf.name_scope('Tower_%d' % (i)) as scope:
                ...
                reduced_loss = loss_hg5 + loss_hg6 + loss_hg7 + loss_hg8 # + loss_hg1 + loss_hg2 + loss_hg3 + loss_hg4
                
                all_trainable_variable = tf.trainable_variables()
		trainable_variable = [v for v in all_trainable_variable if 'Stack1' not in v.name and 'Stack2' not in v.name and 'Stack3' not in v.name and 'Stack4' not in v.name]
